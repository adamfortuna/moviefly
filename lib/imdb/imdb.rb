class Imdb
  ...
  def self.find_movie_by_id(id)
    ...
    infos.each do |info|
      ...
      when "Country:"
        movie.countries = parse_countries(info)
      when "Language:"
        movie.languages = parse_languages(info)
      when "Release Date:"
        begin
  ...
  def self.parse_countries(countries)
    countries.inner_html.scan(/<a href="\/Sections\/Countries\/([^"]+)\/">\n([^<]+)<\/a>?/).map do |match|
      match[1]
    end
  end

  def self.parse_languages(languages)
    languages.inner_html.scan(/<a href="\/Sections\/Languages\/([^"]+)\/">\n([^<]+)<\/a>?/).map do |match|
      match[1]
    end
  end
